...
export async function setupZora(
  ...
  const market = await (await new FreeemaMarketFactory(wallet).deploy()).deployed()
  const marketAddress = market.address

  const media = await (await new FreeemaMediaFactory(wallet).deploy(market.address)).deployed()
