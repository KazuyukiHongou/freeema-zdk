...
export class Zora {
  ...
  public media: FreeemaMedia
  public market: FreeemaMarket
  ...
  constructor(
    ...
    this.media = FreeemaMediaFactory.connect(this.mediaAddress, signerOrProvider)
    this.market = FreeemaMarketFactory.connect(this.marketAddress, signerOrProvider)
  }
  ...
  public async itemIdToItemSales(
    itemId: number
  ): Promise<{
    creator: string
    data: {
    ...
    suppliedCounts: BigNumber
    ask: { amount: BigNumber; currency: string; 0: BigNumber; 1: string }
    onSale: boolean
    0: string
    1: {
      tokenURI: string
    ...
    3: BigNumber
    4: BigNumber
    5: { amount: BigNumber; currency: string; 0: BigNumber; 1: string }
    6: boolean
  }> {
    return this.media.itemIdToItemSales(itemId)
  }
  ...
  /**
   * Sets an item on the an instance of the Zora Media Contract
   * @param contentHash
   * @param supplyLimit
   */
  public async addItem(
    ...
    try {
      ...
      if (supplyLimit > 999999 || supplyLimit < 1) {
        invariant(false, `supplyLimit is invalid`)
      }
    } catch (err) {
      return Promise.reject(err.message)
    }
    return this.media.addItem(contentHash, supplyLimit)
  }

  /**
   * Sets an item and sale on the an instance of the Zora Media Contract
   * @param mediaData
   * @param bidShares
   * @param ask
   * @param supplyLimit
   */
  public async addItemSale(
    mediaData: MediaData,
    bidShares: BidShares,
    ask: Ask,
    supplyLimit: BigNumberish
    ...
    return this.media.addItemSale(mediaData, bidShares, ask, supplyLimit)
  }
   ...
   * @param sig
   */
  public async addItemSaleWithSig(
    creator: string,
    mediaData: MediaData,
    bidShares: BidShares,
    ask: Ask,
    ...
    return this.media.addItemSaleWithSig(
      creator,
      mediaData,
      bidShares,
      sig,
